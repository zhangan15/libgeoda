...
guerry_path <- "../../data/natregimes.shp"
...
Now, with the libgeoda object `guerry_gda`, you can call GeoDa's spatial analysis functions easily. For example, we want to examine the local Moran of variable "HR90" (the homicide):

```{r}
queen_w <- guerry_gda$CreateContiguityWeights()
hr_lisa <- guerry_gda$LISA(queen_w, guerry_sp$HR90)
hr_lisa_clusters <- hr_lisa$GetClusterIndicators()
```

Now, with the LISA results, we can do exploratory spatial data analysis by generating a LISA cluster map:

```{r}
#spplot(guerry_sp, 'HR90')
lisa_color <- c("#eeeeee","#FF0000","#0000FF","#f4ada8","#a7adf9", "#999999")
plot(guerry_sp, col=sapply(hr_lisa_clusters, function(x){return(lisa_color[[x+1]])}))
title(main = "Local Moran Map")
legend('bottomright', legend = c("Not Sig", "High-High", "Low-Low", "High-Low", "Low-High"), fill = lisa_color)
```
From the above code, you can see that we still use `sp` object to do plotting. The values of cluster indicators from `libgeoda`'s `LISA` object are used to make the LISA map. If you check the values of the cluster indicators, you will see they are integer numbers 0 (not significant), 1 (high-high cluster), 2 (low-low cluster), 3 (high-low cluster), 4 (low-high cluster), 5 (neighborless/island), 6 (undefined), which are excatly the same with GeoDa software when you save LISA results to a table:

```{r}
hr_lisa_clusters[1:100]
```

We also specify what color is associated with which cluster value by using following code:
```
lisa_color <- c("#eeeeee","#FF0000","#0000FF","#f4ada8","#a7adf9", "#999999", "#000000")
```
We then create an array of hex color, each observation is set to a hex color according to its cluster indicator value.
```
sapply(hr_lisa_clusters, function(x){return(lisa_color[[x+1]])})
```
